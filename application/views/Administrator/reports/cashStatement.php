        ...
        <div class="row" style="padding-top:10px;">
            <div class="col-md-6">
                ...
                <table class="table table-bordered table-condensed">
                    <thead>
                        <tr style="background: #dee4dc;">
                            <th colspan="4">Cash Fund Received</th>
                        </tr>
                        <tr>
                            <th>Sl</th>
                            <th>Transfer from</th>
                    ...
                    <tbody>
                        <tr v-for="transaction in fundReceives">
                            <td>{{ transaction.transfer_id }}</td>
                            <td>{{ transaction.transfer_from_name }}</td>
                            <td>{{ transaction.transfer_date }}</td>
                            <td style="text-align:right;">{{ transaction.transfer_amount | decimal }}</td>
                        </tr>
                    </tbody>
                    <tfoot>
                        <tr style="font-weight:bold;">
                            <td colspan="3" style="text-align:right">Total</td>
                            <td style="text-align:right;">
                                <span>{{ totalFundReceived | decimal }}</span>
                            </td>
                        </tr>
                    </tfoot>
            ...
            <div class="col-md-6">
                ...
                <table class="table table-bordered table-condensed">
                    ...
                    <tbody>
                        <tr v-for="transaction in fundTransfers">
                            <td>{{ transaction.transfer_id }}</td>
                            <td>{{ transaction.transfer_to_name }}</td>
                            <td>{{ transaction.transfer_date }}</td>
                            <td style="text-align:right;">{{ transaction.transfer_amount | decimal }}</td>
                        </tr>
                    </tbody>
                    <tfoot>
                        <tr style="font-weight:bold;">
                            <td colspan="3" style="text-align:right">Total</td>
                            <td style="text-align:right;">
                                <span>{{ totalFundTransfer | decimal }}</span>
                            </td>
                        </tr>
                    </tfoot>
                </table>
...
new Vue({
    ...
    computed: {
        ...
        totalFundTransfer() {
            return this.fundTransfers.reduce((prev, curr) => {
                return prev + parseFloat(curr.transfer_amount)
            }, 0).toFixed(2);

        },
        totalFundReceived() {
            return this.fundReceives.reduce((prev, curr) => {
                return prev + parseFloat(curr.transfer_amount)
            }, 0).toFixed(2);
        },
        totalCashIn() {
            return parseFloat(this.totalSales) +
                ...
                parseFloat(this.totalFundReceived) +
        ...
        totalCashOut() {
            return parseFloat(this.totalPurchase) +
                ...
                parseFloat(this.totalFundTransfer) +
    ...
    methods: {
        ...
        getSales() {
            axios.post('/get_sales', this.filter)
                .then(res => {
                    this.sales = res.data.sales.filter((e) => {
                        return e.payment_type != 'bank'
                    });
                })
        },
        ...
        getPurchases() {
            axios.post('/get_purchases', this.filter)
                .then(res => {
                    this.purchases = res.data.purchases.filter((e) => {
                        return e.payment_type != 'bank';
                    });
                })
        },
        ...
        getFundTransfers() {
            ...
            let filter = {
                dateFrom: this.filter.dateFrom,
                dateTo: this.filter.dateTo,
                branchId: this.selectedBranch != null && this.selectedBranch.brunch_id != '' ? this
                    .selectedBranch.brunch_id : null
            }

            axios.post('/get_cash_transfers', filter).then(res => {
                this.fundTransfers = res.data.filter((e) => {
                    return e.status != 'd';
                });
            })
        },
        ...
        getFundReceives() {
            ...
            axios.post('/get_cash_receives', filter).then(res => {
                this.fundReceives = res.data.filter((e) => {
                    return e.status != 'p';
                });
            })
        },
