                    ...
                    <div style="width:50%;border:1px solid black;position:relative;">
                        <table class="day-book-table">
                            ...
                            <tbody>
                                ...
                                <template v-if="cashTransferReceived.length > 0">
                                    <tr>
                                        <td class="sub-heading">Cash Transfer Received</td>
                                        <td class="sub-value">{{ totalCashFundReceived }}</td>
                                    </tr>
                                    <tr v-for="transaction in cashTransferReceived">
                                        <td>{{ transaction.Brunch_name }}</td>
                                        <td>{{ transaction.totalAmount | decimal }}</td>
                                    </tr>
                                </template>
                    ...
                    <div style="width:50%;border:1px solid black;border-left:none;position:relative;">
                        <table class="day-book-table">
                            ...
                            <tbody>
                                ...
                                <template v-if="cashTransfer.length > 0">
                                    <tr>
                                        <td class="sub-heading">Cash Transfer Payment</td>
                                        <td class="sub-value">{{ totalCashTransfer }}</td>
                                    </tr>
                                    <tr v-for="transaction in cashTransfer">
                                        <td>{{ transaction.Brunch_name }}</td>
                                        <td>{{ transaction.totalAmount | decimal }}</td>
                                    </tr>
                                </template>
...
new Vue({
    ...
    computed: {
        ...
        totalCashFundReceived() {
            return this.cashTransferReceived.reduce((prev, curr) => {
                return prev + parseFloat(curr.transfer_amount)
            }, 0).toFixed(2);
        },
        totalCashTransfer() {
            return this.cashTransfer.reduce((prev, curr) => {
                return prev + parseFloat(curr.transfer_amount)
            }, 0).toFixed(2);
        },
        ...
        totalIn() {
            return parseFloat(this.openingBalance.cashBalance.cash_balance) +
                ...
                parseFloat(this.totalCashFundReceived) +
        ...
        totalOut() {
            return parseFloat(this.totalPurchase) +
                ...
                parseFloat(this.totalCashTransfer) +
    ...
    methods: {
        ...
        async getCashTransferReceived() {
            let filter = {
                dateFrom: this.filter.dateFrom,
                dateTo: this.filter.dateTo,
            }
            await axios.post('/get_cash_transfer_received', filter)
                .then(res => {
                    let transaction = res.data.filter((e) => {
                        return e.status != 'p'
                    });
                    ...
                    transactions = transactions.map(transaction => {
                        transaction[0].totalAmount = transaction.reduce((p, c) => {
                            return p + parseFloat(c.transfer_amount)
                        }, 0);
                        return transaction[0];
                    })
                    this.cashTransferReceived = transactions;

                })

        },

        async getCashTransfer() {

            let filter = {
                dateFrom: this.filter.dateFrom,
                dateTo: this.filter.dateTo,
                userFullName: this.selectedUser != null ? this.selectedUser.User_SlNo : null,
            }
            ...
            await axios.post('/get_cash_transfers', filter)
                .then(res => {
                    ...
                    this.cashTransfer = transactions;
                })
        },
